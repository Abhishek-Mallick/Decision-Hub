...
const factsButton = [{ label : 'Add rules', disable: false },
...
class AddDecision extends Component {
    ...
    topPanel() {
        ...
        <div className="step2"><div> Step 2: Add / Remove rules</div><ButtonGroup buttons={factsButton} onConfirm={this.handleChildrenNode} /></div>
        ...
        return (<Panel>
            ...
            <div className={`attributes-header ${background}`}>
                    <div className="attr-link" onClick={this.addPath}>
                        <span className="plus-icon" /><span className="text"></span> 
                    </div>
            </div>

            <div className="add-field-panel">
                <div><SelectField options={attributeOptions} onChange={(e) => this.onChangeNewFact(e, 'name')}
                        value={addAttribute.name} error={addAttribute.error.name} label="LHS"/></div>
                ...
                <div><InputField onChange={(value) => this.onChangeNewFact(value, 'value')} value={addAttribute.value}
                        error={addAttribute.error.value} label="RHS" placeholder={placeholder}/></div>
            </div>
